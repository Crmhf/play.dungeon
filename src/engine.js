...
const HERO_TYPES = {
  knight: { name:'骑士', desc:'近战肉盾 · 横扫一片', sheet:'chars', col:0, row:0, hp:240, spd:150, atk:30, rate:0.50, range:95,  arc:true,  projSpeed:0,   dash:1.5, color:'#c8d0e0', sfx:'sword',
    passive:{ key:'damageReduce', val:0.15, name:'圣盾体质', desc:'受伤减免15%' },
    skill:{ name:'旋风斩', cd:7, color:'#9adcff', desc:'横扫+减速敌人+3秒减伤50%' } },
  archer: { name:'弓手', desc:'远程安全输出 · 穿透箭', sheet:'chars', col:6, row:3, hp:115, spd:165, atk:11, rate:0.36, range:500, arc:false, projSpeed:520, pierce:true, dash:1.0, color:'#a8e063', sfx:'bow',
    passive:{ key:'critChance', val:0.18, name:'鹰眼', desc:'天生暴击率18%' },
    skill:{ name:'天降箭雨', cd:9, color:'#a8e063', desc:'目标区域倾泻24支落箭' } },
  mage:   { name:'法师', desc:'中距AOE · 弹射灼烧', sheet:'priest', variant:1, hp:100, spd:140, atk:14, rate:0.50, range:440, arc:false, projSpeed:420, bounce:2, aoe:60, dash:0.9, color:'#b06ce0', sfx:'magic',
    passive:{ key:'skillCdMul', val:0.8, name:'奥术亲和', desc:'技能冷却-20%' },
    skill:{ name:'奥术湮灭', cd:12, color:'#e08cff', desc:'大爆炸+灼烧3秒,毁天灭地' } },
  rogue:  { name:'盗贼', desc:'近战爆发 · 刀刀烈火', sheet:'chars', col:0, row:1, hp:115, spd:195, atk:15, rate:0.18, range:80,  arc:true,  projSpeed:0,   dodgeCd:1.3, dash:1.3, color:'#e0b25c', sfx:'swing',
...
function startWave(){
  ...
  if(isBossWave){
    ...
    const dmgMul = 1 + (G.wave-1)*0.06;          // 递增伤害(避免后期挠痒痒)
...
function updatePlayer(p,dt){
  ...
  if(l>0){
    ...
    const engage = (p.def.arc? 150 : p.def.range*0.9) * p.rangeMul; // 远程交战距离随射程
...
function updateAIPlayer(p,dt){
  ...
  if(tgt){
    ...
    const desired = p.def.arc? 50 : p.def.range*0.65; // 近战贴近 / 远程按射程保持风筝距离
